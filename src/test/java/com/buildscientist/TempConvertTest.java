...
public class TempConvertTest {
	...
	private final static Logger logger = LoggerFactory.getLogger(TempConvertTest.class);
	...
	@Ignore
	@Test
	public void mockCelciusToFarenheight() {
		String conversionEndpointURI = "/tempconvert.asmx?op=CelsiusToFarenheight";	
		String xpathQuery = "//soapenv:Envelope | //soapenv:Body | //CelsiusToFahrenheitResponse | //CelsiusToFahrenheitResult";
		...
		wiremockRule.stubFor(post(urlEqualTo(conversionEndpointURI)).withRequestBody(
				matchingXPath(xpathQuery)).willReturn(
				aResponse().withStatus(HttpStatus.ORDINAL_200_OK).withBody("")));
		
		
		wiremockRule.verify(postRequestedFor(urlEqualTo(conversionEndpointURI)).withRequestBody(matchingXPath(xpathQuery)));
